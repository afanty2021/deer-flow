...
- **IM 通道**: Telegram、Slack、Feishu/Lark、Discord、WeChat 集成（流式更新、文件附件上传）
...
- **单元测试**: 位于 `tests/unit/` 下，镜像 `src/` 布局
- **CI 集成**: GitHub Actions workflow (`.github/workflows/frontend-unit-tests.yml`)
- **测试文件示例**:
  - `tests/unit/core/api/stream-mode.test.ts` - API 流式模式测试
  - `tests/unit/core/threads/utils.test.ts` - 线程工具测试
  - `tests/unit/core/uploads/file-validation.test.ts` - 文件验证测试
  - `tests/unit/core/uploads/prompt-input-files.test.ts` - 提示输入文件测试
...
- `smoke-test` - 烟雾测试
- `surprise-me` - 随机创意
- `systematic-literature-review` - 系统性文献综述（多论文 SLR 工作流）
...
### 2026-04-13 - 合并上游更新（基于 30+ 个新提交）

**新功能**:
- **Vitest 测试框架**: 前端添加完整的单元测试基础设施，包含 CI workflow (#2147)
- **熔断器**: 引入轻量级熔断器防止速率限制和资源耗尽 (#2095)
- **Discord 通道集成**: 添加 Discord IM 通道支持 (#1806)
- **WeChat 通道集成**: 添加微信 IM 通道支持 (#1869)
- **子代理模型覆盖**: 允许在 config.yaml 中为每个子代理覆盖模型配置 (#2064)
- **Ollama 思考支持**: 添加 langchain-ollama 用于原生 Ollama thinking 支持 (#2062)
- **系统性文献综述技能**: 添加 systematic-literature-review 技能用于多论文 SLR 工作流 (#2032)
- **烟雾测试技能**: 添加 smoke-test 技能 (#1947)
- **PVC 支持**: K8s 沙箱卷添加可选 PVC 支持 (#2020)
- **博客结构**: 实现博客结构，包含帖子列表、标签和布局增强 (#1962)
- **Setup Wizard + doctor**: 添加设置向导和诊断命令，改进开发者体验 (#2034)
- **检查点完整回滚**: 在用户取消时实现完整检查点回滚 (#1867)

**重要修复**:
- **循环检测改进**: 为 LoopDetectionMiddleware 添加每种工具类型的频率检测 (#1988)
- **沙箱安全**: 改进沙箱安全并保留多模态内容 (#2114)
- **路径解析**: 解析 LocalSandbox 中 read_file/write_file 内容的路径 (#1935)
- **内存泄漏**: 使用 WeakValueDictionary 防止文件操作锁中的内存泄漏 (#2096)
- **容器启动协调**: 添加启动协调以防止孤立容器泄漏 (#1976)
- **Dangling tool-call 修复**: 修复循环中断后 dangling tool-call 历史记录 (#2035)
- **Windows 支持**: 路由 Windows shell 脚本目标通过 Git Bash (#2060)

**前端改进**:
- **无效字段修复**: 将 threads.search 中的无效 "context" 选择字段替换为 "metadata" (#2053)
- **DeerFlowClient 流**: 将 DeerFlowClient AI 文本流式传输为 token delta (#1969)

**后端改进**:
- **Uvicorn 选项**: 更新 CMD 使用 --no-sync 选项运行 uvicorn (#2100)

**文档更新**:
- **行为准则**: 添加 Contributor Covenant Code of Conduct
- **API 文档**: 记录 LangGraph API 运行的 recursion_limit (#1929)
- **异步迁移**: 将已完成的异步迁移移动到已完成功能 (#2146)

**依赖更新**:
- **langsmith**: 从 0.5.2 升级到 0.5.18 (frontend) (#2110)
- **langchain-core**: 从 1.2.17 升级到 1.2.28 (backend) (#2109)
