...
Bridges external messaging platforms (Feishu, Slack, Telegram, Discord, WeChat, DingTalk) to the DeerFlow agent via Gateway's LangGraph-compatible API.
...
- `manager.py` - Core dispatcher: creates threads via `client.threads.create()`, routes commands, keeps Slack/Telegram/Discord/WeChat/DingTalk on `client.runs.wait()`, and uses `client.runs.stream(["messages-tuple", "values"])` for Feishu incremental outbound updates
- `base.py` - Abstract `Channel` base class (start/stop/send lifecycle)
- `service.py` - Manages lifecycle of all configured channels from `config.yaml`
- `slack.py` / `feishu.py` / `telegram.py` / `discord.py` / `wechat.py` / `dingtalk.py` - Platform-specific implementations (`feishu.py` tracks the running card `message_id` in memory and patches the same card in place; `dingtalk.py` optionally uses AI Card streaming for in-place updates when `card_template_id` is configured)
...
5. Slack/Telegram/Discord/WeChat/DingTalk chat: `runs.wait()` → extract final response → publish outbound
